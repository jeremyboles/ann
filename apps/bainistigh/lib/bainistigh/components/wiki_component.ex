      ...
      <details class="tags_input" open>
        ...
        <ul id="wiki-tags" phx-hook="TagList">
          <%= for {tag, index} <- Enum.with_index(input_value(@form, :tags)) do %>
            <li>
              <input autocomplete="off" name={"#{input_name(@form, :tags)}[]"} id={input_id(@form, :tags, index)} phx-keydown="modify-tags" phx-value-index={index} type="text" value={tag} />
            </li>
          <% end %>
        </ul>
      </details>
    ...
    ~H"""
      <details class="topic_attributes" open>
        ...
        <fieldset>
          ...
          <.live_component module={Bainistigh.WikiLive.ParentTopicComponent} form={@form} id="parent-topic" />
        </fieldset>
      </details>
    """
  end
